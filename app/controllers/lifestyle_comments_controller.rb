class LifestyleCommentsController < ApplicationController
  before_action :move_to_sign_up, only: [:create]

  def index
    @comment = Comment.new
    @title = Title.find(params[:lifestyle_title_id])
  ...
  def create
    ...
    if @comment.save
      redirect_to lifestyle_title_lifestyle_comments_path(@title)
    else
