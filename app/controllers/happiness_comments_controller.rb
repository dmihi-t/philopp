class HappinessCommentsController < ApplicationController
  before_action :move_to_sign_up, only: [:create]

  def index
    @comment = Comment.new
    @title = Title.find(params[:happiness_title_id])
  ...
  def create
    ...
    if @comment.save
      redirect_to happiness_title_happiness_comments_path(@title)
    else
