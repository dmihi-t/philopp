class OccupationCommentsController < ApplicationController
  before_action :move_to_sign_up, only: [:create]

  def index
    @comment = Comment.new
    @title = Title.find(params[:occupation_title_id])
  ...
  def create
    ...
    if @comment.save
      redirect_to occupation_title_occupation_comments_path(@title)
    else
